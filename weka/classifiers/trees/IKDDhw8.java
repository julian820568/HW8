...
public class IKDDhw8
  ...
  /** Successor nodes. */
  protected IKDDhw8[] m_Successors;
  ...
  protected void makeTree(Instances data, int totalInstances, int[][] sortedIndices,
	...
	return;
      ...
      m_isLeaf = false;
      m_Successors = new IKDDhw8[2];
      for (int i = 0; i < 2; i++) {
	m_Successors[i] = new IKDDhw8();
  ...
  public void prune(double alpha) throws Exception {
    ...
    while (prune) {
      ...
      IKDDhw8 nodeToPrune = nodeToPrune(nodeList);
...
//	  splitPoint = Math.rint((inst.value(att) + currSplit)/2.0*100000)/100000.0;
   ...
   */
  protected IKDDhw8 nodeToPrune(Vector nodeList) {
    if (nodeList.size()==0) return null;
    if (nodeList.size()==1) return (IKDDhw8)nodeList.elementAt(0);
    IKDDhw8 returnNode = (IKDDhw8)nodeList.elementAt(0);
    double baseAlpha = returnNode.m_Alpha;
    for (int i=1; i<nodeList.size(); i++) {
      IKDDhw8 node = (IKDDhw8)nodeList.elementAt(i);
  ...
  protected double computeGini(double[] dist, double total) {
    ...
    for (int i=0; i<dist.length; i++) {
      val += (dist[i]/total)*Math.log(dist[i]/total);
    }
    return (-val);
  }
  /*protected double computeGini(double[] dist, double total) {
  ...
  public static void main(String[] args) {
    runClassifier(new IKDDhw8(), args);
  }
}
